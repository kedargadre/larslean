...
def run_shap_explanation(
    ...
    try:
        from ml.explainability import explain_score
        feature_vector = X.loc[[row_idx]]
        drivers = explain_score(model, feature_vector, available_features, top_n=5)
        return drivers
    except Exception as e:
...
def run_rent_forecast(
    ...
    result = forecast_metric(series, n_periods=n_periods)
    ...
    return {
        ...
        "lower_ci": [round(float(v)) for v in result["lower_ci"]],
        "upper_ci": [round(float(v)) for v in result["upper_ci"]],
    }
